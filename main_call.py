...
active_connections_call: Dict[str, WebSocket] = {}
...
async def call_signaling(websocket: WebSocket, user_id: str):
    await websocket.accept()
    print(f"📞 Call WebSocket connected: {user_id}")
    active_connections_call[user_id] = websocket

    try:
        while True:
            data = await websocket.receive_text()
            print(f"📥 From {user_id}: {data}")
            message = json.loads(data)

            target_id = message.get("target")
            if not target_id:
                print(f"⚠️ Missing target in message from {user_id}")
                continue

            if target_id in active_connections_call:
                await active_connections_call[target_id].send_text(json.dumps({
                    "from": user_id,
                    "type": message.get("type"),
                    "data": message.get("data")
                }))
    except WebSocketDisconnect:
        print(f"❌ Call WebSocket disconnected: {user_id}")
        active_connections_call.pop(user_id, None)
