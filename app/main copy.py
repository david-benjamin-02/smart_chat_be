...
# # --- Speech to Text ---
# async def speech_to_texts(audio: UploadFile = File(...)):
#     try:
...
#         # Generate a MongoDB-style ObjectId for unique filename
#         unique_filename = f"{str(ObjectId())}.mp3"
#         audio_store = os.path.join(
#             "static/uploads", unique_filename
#         )  # server file path
#         audio_path = f"/uploads/{unique_filename}"  # client-accessible path

#         # Save the TTS audio
#         gTTS(text=text, lang=language).save(audio_store)

#         return {"audio_file": audio_path}
#     except Exception as e:
#         print("❌ Audio generation error:", e)  # ✅ log the error
#         return {
...
#         {"receiver_id": sender_id, "status": "sent"}, {"$set": {"status": "delivered"}}
#     )

#     # Find all messages just marked as delivered
#     undelivered_messages = await chat_collection.find(
#         {"receiver_id": sender_id, "status": "delivered"}
#     ).to_list(None)

#     # Notify the original senders (if they're online)
...
#                 print(f"⚠️ Failed to notify {original_sender}: {e}")
#                 # Optionally remove broken socket
...
#                         message_format == "None" or message_format == "Audio"
#                     ):
#                         pass
...
#                         trans_text = await translate_texts(
#                             transcribe_message, receiver_lang
#                         )
#                         trans_message = trans_text["translated_text"]
#                     elif receiver_lang and (
#                         message_format == "None" or message_format == "Audio"
#                     ):
#                         transcribe_message = message.get("transformed_text")
#                         print("transcribe_message", transcribe_message)
#                         trans_text = await translate_texts(
#                             transcribe_message, receiver_lang
#                         )
#                         trans_message = trans_text["translated_text"]
#                         audio_result = await text_to_audios(
#                             trans_message, receiver_lang
#                         )
#                         trans_message=""
#                         if (
#                             isinstance(audio_result, dict)
#                             and "audio_file" in audio_result
#                         ):
#                             trans_media_url = audio_result["audio_file"]  # ✅ corrected
...
#                                 trans_message, receiver_lang
#                             )
#                             print("text_to_audios", audio_result)
#                             # Check if it's a dict with "audio_file"
#                             if (
#                                 isinstance(audio_result, dict)
#                                 and "audio_file" in audio_result
#                             ):
#                                 trans_media_url = audio_result[
#                                     "audio_file"
#                                 ]  # ✅ corrected
#                             else:
#                                 trans_media_url = ""
...
#                     # message["name"] = sender_user["email"] if sender_user else sender_id
#                     # await active_connections[receiverId].send_text(json.dumps(message))
#                     sender_user = await user_collection.find_one(
#                         {"uid": sender_id}, {"email": 1}
#                     )
#                     await active_connections[receiverId].send_text(
#                         json.dumps(
#                             {
#                                 "id": messageId,
#                                 "msg_text": msg_text,
...
#                                 "time": time,
#                                 "name": (
#                                     sender_user["email"] if sender_user else sender_id
#                                 ),
#                                 "transformation": {
#                                     "message_type": message_format,
#                                     "trans_message": trans_message,
...
#                         {"message_id": messageId}, {"$set": {"status": "delivered"}}
#                     )
...
#                 )
#                 await chat_collection.update_one(
#                     {"message_id": message_id}, {"$set": {"status": "read"}}
#                 )
...
# async def call_signaling(websocket: WebSocket, user_id: str):
#     await websocket.accept()
...
#                 await active_connections[target_id].send_text(
#                     json.dumps(
#                         {
#                             "from": user_id,
...
#         # --- Fetch receiver's message format ---
#         receiverSettings = await fetch_receiver_data(receiverId)
#         message_format = receiverSettings.get("message_format")
#         receiver_lang = receiverSettings.get("receiver_lang")
#         trans_message = ""
#         print(receiver_lang)
#         # --- Transcribe only if receiver prefers text ---
#         if message_format == "Text" or receiver_lang:
#             print("🧠 Transcribing voice for receiver with Text format...")

#             # Create a new UploadFile from saved content
#             fake_upload = StarletteUploadFile(
#                 filename=filename, file=io.BytesIO(content)
#             )

#             result = await speech_to_texts(fake_upload)
